...
def test_event_init_fields():
    event= TelemetryEvent.create("character_attack", 0, 1,"s1","anon_user",{"damage":5})
...
def test_event_id_unique():
    event1 = TelemetryEvent.create("event", 0, 1, "s1", "u1", {})
    event2 = TelemetryEvent.create("event", 0, 1, "s1", "u1", {})
...
def test_mutable_defaults_independent():
    event1=TelemetryEvent.create("event1", 0, 1,"s1", "u1",{})
    event2=TelemetryEvent.create("event2", 0, 2,"s2","u2",{})
...
def test_event_type_wrong_type():
    with pytest.raises(TypeError):
        TelemetryEvent.create(123, 0, 1,"s1", "u1", {})

def test_level_id_wrong_type():
    with pytest.raises(TypeError):
        TelemetryEvent.create("event", "wrong_level", 0, "s1", "u1",{})

def test_stage_id_wrong_type():
    with pytest.raises(TypeError):
        TelemetryEvent.create("event", 0, "wrong_stage","s1", "u1",{})
